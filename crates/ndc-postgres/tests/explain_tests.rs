...
async fn select_by_pk() {
    let result = run_explain(CHINOOK_DEPLOYMENT_PATH, "select_by_pk").await;
...
async fn select_where_variable() {
    let result = run_explain(CHINOOK_DEPLOYMENT_PATH, "select_where_variable").await;
...
async fn select_where_name_nilike() {
    let result = run_explain(CHINOOK_DEPLOYMENT_PATH, "select_where_name_nilike").await;
